...
def load_images_from_folder(folder):
    ...
    for filename in os.listdir(folder):
        ...
        if img is not None:
            img = cv.normalize(img, None, -1, 1, norm_type=cv.NORM_MINMAX, dtype=cv.CV_32F)
            images[i]=img
            i+=1
...
images = images[::3,250:326:4,300:376:4] #take the central part where 4 colors are visible, and sample every 4 pixels to make it "low res"
...
yRGB = cv.normalize(yRGB, None, np.amin(images), np.amax(images), norm_type=cv.NORM_MINMAX, dtype=cv.CV_32F)

Z = np.dot(A, H)
print(Z.shape) #yay

Z = np.reshape(Z, (Z.shape[0], 76,76))
print(Z.shape)
print("A: ", A)
print("X: ", X)
print("yRGB: ", yRGB)
...
savemat('bpnd_params.mat', mdict={'yRGB': np.reshape(np.transpose(yRGB), (76,76,3)), 'yhs' : np.reshape(np.transpose(y), (h, w, S)), 'A' : A, 'X' : X})
